    ...
    items: [
      {
        label: 'Ventas',
        children: [
          { label: 'Ventas Generales', path: '/reportes/ventas/generales' },
          { label: 'Ventas por Cliente', path: '/reportes/ventas/cliente' },
          { label: 'Ventas por Producto', path: '/reportes/ventas/producto' },
        ],
      },
      {
        label: 'Compras',
        children: [
          { label: 'Compras Generales', path: '/reportes/compras/generales' },
          { label: 'Compras por Proveedor', path: '/reportes/compras/proveedor' },
        ],
      },
...
export default function AppLayout() {
  ...
  const getAvatarSrc = () => {
    ...
    // Si ya viene con prefijo data:image o es una URL http/https, se usa directamente
    if (foto.startsWith('data:image') || foto.startsWith('http')) {
      return foto;
    }
    // Si es un string base64 sin prefijo, se asume que es PNG
    if (foto.length > 0) {
      // A veces la base64 viene con espacios o saltos de línea, los removemos
