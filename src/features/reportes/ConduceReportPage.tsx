...
import { reporteApi, type ConduceReporteDto } from '../../api/reporte.api';
...
export default function ConduceReportPage() {
  ...
  const [reporte, setReporte] = useState<ConduceReporteDto[]>([]);
  ...
  const cargarReporte = async () => {
    ...
    setError('');
    try {
      const { data } = await reporteApi.conduce(filtros.fecha1, filtros.fecha2, filtros.idCliente);
      setReporte(data);
    } catch (err: any) {
  ...
  const exportarPDF = async () => {
    ...
    doc.text('REPORTE DE CONDUCE', pw / 2, y + 35, { align: 'center' });
    ...
    const cols = [25, 25, 30, 60, 30, 30, 30, 35];
    const headers = ['Fecha', 'No. Conduce', 'RNC/Cédula', 'Cliente', 'Subtotal', 'ITBIS', 'Descuento', 'Total'];
    let x = 15;
    ...
    reporte.forEach((row, idx) => {
      ...
      doc.text(row.no_Conduce, x + 2, y + 4); x += cols[1];
      doc.text(row.num_Documento || '-', x + 2, y + 4); x += cols[2];
      doc.text(row.cliente.substring(0, 30), x + 2, y + 4); x += cols[3];
      ...
      doc.text(formatMoney(row.total), x + cols[7] - 5, y + 4, { align: 'right' });
    ...
    doc.text(formatMoney(tItbis), pw - 85, y + 5, { align: 'right' });
    doc.text(formatMoney(tTotal), pw - 20, y + 5, { align: 'right' });

    doc.save(`Reporte_Conduce_${filtros.fecha1}_al_${filtros.fecha2}.pdf`);
  };

  const exportarExcel = async () => {
    if (reporte.length === 0) return;
    const wb = new ExcelJS.Workbook();
    const ws = wb.addWorksheet('Conduce');
    ...
    ws.mergeCells('A3:H3'); ws.getCell('A3').value = 'REPORTE DE CONDUCE';
    ws.getCell('A3').font = { size: 14, bold: true }; ws.getCell('A3').alignment = { horizontal: 'center' };

    const hr = ws.addRow(['Fecha', 'No. Conduce', 'RNC/Cédula', 'Cliente', 'Subtotal', 'ITBIS', 'Descuento', 'Total']);
    hr.height = 20;
    hr.eachCell(c => {
    ...
    let tSub = 0, tItbis = 0, tDesc = 0, tTotal = 0;
    reporte.forEach((row, idx) => {
      const dr = ws.addRow([formatDate(row.fecha), row.no_Conduce, row.num_Documento || '-', row.cliente, row.subtotal, row.itbis, row.descuento, row.total]);
      dr.height = 18;
      dr.eachCell((c, n) => {
    ...
    ws.columns = [{ width: 12 }, { width: 15 }, { width: 18 }, { width: 35 }, { width: 15 }, { width: 15 }, { width: 15 }, { width: 15 }];

    const buf = await wb.xlsx.writeBuffer();
    saveAs(new Blob([buf]), `Reporte_Conduce_${filtros.fecha1}_al_${filtros.fecha2}.xlsx`);
  };
        ...
        <div className="bg-gradient-to-r from-emerald-800 to-emerald-700 rounded-xl shadow-lg overflow-hidden">
          <div className="px-6 py-4 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-white">Relación de Conduce</h1>
            ...
            <>
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 p-4 bg-gradient-to-r from-emerald-50 to-yellow-50 border-b border-emerald-100">
                <div className="bg-white rounded-lg p-3 shadow-sm">
                  <p className="text-xs text-gray-500 uppercase font-semibold">Total Conduce</p>
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gradient-to-r from-emerald-700 to-emerald-600 text-white">
                    <tr>
                      ...
                      <th className="px-3 py-3 text-left font-semibold">No. Conduce</th>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {reporte.map((row, idx) => (
                      <tr key={idx} className={`hover:bg-emerald-50 transition-colors ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
                        ...
                        <td className="px-3 py-2 font-mono font-bold text-emerald-700">{row.no_Conduce}</td>
