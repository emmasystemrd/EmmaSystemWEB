...
import { reporteApi, type ComisionVentaReporteDto } from '../../api/reporte.api';
import { empresaApi, type EmpresaDto } from '../../api/empresa.api';
import { useAuthStore } from '../../store/authStore';

export default function ComisionVentaReportPage() {
  ...
  const [reporte, setReporte] = useState<ComisionVentaReporteDto[]>([]);
  ...
  const cargarReporte = async () => {
    ...
    setError('');
    try {
      const { data } = await reporteApi.comisionVenta(filtros.fecha1, filtros.fecha2);
      setReporte(data);
    } catch (err: any) {
  ...
  const exportarPDF = async () => {
    ...
    doc.text('REPORTE DE COMISIÓN POR VENTAS', pw / 2, y + 35, { align: 'center' });
    ...
    const headers = ['Código', 'Empleado', '% Comisión', 'Ventas', 'Comisión'];
    let x = 15;
    ...
    reporte.forEach((row, idx) => {
      ...
      doc.text(row.empleado.substring(0, 40), x + 2, y + 4); x += cols[1];
    ...
    doc.save(`Reporte_Comision_Venta_${filtros.fecha1}_al_${filtros.fecha2}.pdf`);
  };

  const exportarExcel = async () => {
    if (reporte.length === 0) return;
    const wb = new ExcelJS.Workbook();
    const ws = wb.addWorksheet('Comisión Ventas');
    ...
    ws.mergeCells('A3:E3'); ws.getCell('A3').value = 'REPORTE DE COMISIÓN POR VENTAS';
    ws.getCell('A3').font = { size: 14, bold: true }; ws.getCell('A3').alignment = { horizontal: 'center' };

    const hr = ws.addRow(['Código', 'Empleado', '% Comisión', 'Ventas', 'Comisión']);
    hr.height = 20;
    hr.eachCell(c => {
    ...
    let tVentas = 0, tComision = 0;
    reporte.forEach((row, idx) => {
      const dr = ws.addRow([row.codigo, row.empleado, row.porcentaje, row.ventas, row.comision]);
      dr.height = 18;
      dr.eachCell((c, n) => {
    ...
    saveAs(new Blob([buf]), `Reporte_Comision_Venta_${filtros.fecha1}_al_${filtros.fecha2}.xlsx`);
  };
        ...
        <div className="bg-gradient-to-r from-emerald-800 to-emerald-700 rounded-xl shadow-lg overflow-hidden">
          <div className="px-6 py-4 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-white">Comisión por Ventas</h1>
        ...
        <div className="bg-white rounded-xl shadow-md border border-emerald-100 overflow-hidden">
          {loading ? (
            ...
            <div className="p-12 text-center text-gray-500">
              ...
              <p className="text-xs mt-1">Nota: Solo se muestran empleados con artículos que tienen comisión configurada</p>
            </div>
          ) : (
            <>
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3 p-4 bg-gradient-to-r from-emerald-50 to-yellow-50 border-b border-emerald-100">
                <div className="bg-white rounded-lg p-3 shadow-sm">
                  <p className="text-xs text-gray-500 uppercase font-semibold">Total Empleados</p>
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gradient-to-r from-emerald-700 to-emerald-600 text-white">
                    <tr>
                      ...
                      <th className="px-3 py-3 text-left font-semibold">Empleado</th>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {reporte.map((row, idx) => (
                      <tr key={idx} className={`hover:bg-emerald-50 transition-colors ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
                        ...
                        <td className="px-3 py-2 font-medium text-gray-900">{row.empleado}</td>
