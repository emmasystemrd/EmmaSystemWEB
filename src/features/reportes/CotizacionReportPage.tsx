...
import { reporteApi, type CotizacionReporteDto } from '../../api/reporte.api';
import { empresaApi, type EmpresaDto } from '../../api/empresa.api';
import { useAuthStore } from '../../store/authStore';

export default function CotizacionReportPage() {
  ...
  const [reporte, setReporte] = useState<CotizacionReporteDto[]>([]);
  ...
  const cargarReporte = async () => {
    ...
    setError('');
    try {
      const { data } = await reporteApi.cotizaciones(filtros.fecha1, filtros.fecha2, filtros.proceso);
      setReporte(data);
    } catch (err: any) {
  ...
  const exportarPDF = async () => {
    ...
    doc.text('REPORTE DE COTIZACIONES', pw / 2, y + 35, { align: 'center' });
    ...
    const headers = ['Fecha', 'No. Cotiz.', 'RNC/Cédula', 'Cliente', 'Subtotal', 'ITBIS', 'Desc.', 'Total', 'Proceso'];
    let x = 15;
    ...
    reporte.forEach((row, idx) => {
      ...
      doc.text(row.no_Cotizacion, x + 2, y + 4); x += cols[1];
    ...
    doc.save(`Reporte_Cotizaciones_${filtros.fecha1}_al_${filtros.fecha2}.pdf`);
  };
  ...
  const exportarExcel = async () => {
    ...
    ws.mergeCells('A3:I3'); ws.getCell('A3').value = 'REPORTE DE COTIZACIONES';
    ws.getCell('A3').font = { size: 14, bold: true }; ws.getCell('A3').alignment = { horizontal: 'center' };

    const hr = ws.addRow(['Fecha', 'No. Cotización', 'RNC/Cédula', 'Cliente', 'Subtotal', 'ITBIS', 'Descuento', 'Total', 'Proceso']);
    hr.height = 20;
    hr.eachCell(c => {
    ...
    let tSub = 0, tItbis = 0, tDesc = 0, tTotal = 0;
    reporte.forEach((row, idx) => {
      const dr = ws.addRow([formatDate(row.fecha), row.no_Cotizacion, row.num_Documento || '-', row.cliente, row.subtotal, row.itbis, row.descuento, row.total, row.proceso]);
      dr.height = 18;
      dr.eachCell((c, n) => {
    ...
    saveAs(new Blob([buf]), `Reporte_Cotizaciones_${filtros.fecha1}_al_${filtros.fecha2}.xlsx`);
  };
  ...
  const abiertas = reporte.filter(r => r.proceso === 'ABIERTA').length;
  const cerradas = reporte.filter(r => r.proceso === 'CERRADA').length;
      ...
      <div className="max-w-7xl mx-auto space-y-4">
        <div className="bg-gradient-to-r from-emerald-800 to-emerald-700 rounded-xl shadow-lg overflow-hidden">
          <div className="px-6 py-4 flex items-center justify-between">
            <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold text-white">Relación de Cotizaciones</h1>
        ...
        <div className="bg-white rounded-xl shadow-md border border-emerald-100 overflow-hidden">
          {loading ? (
            ...
            <>
              <div className="grid grid-cols-2 md:grid-cols-5 gap-3 p-4 bg-gradient-to-r from-emerald-50 to-yellow-50 border-b border-emerald-100">
                <div className="bg-white rounded-lg p-3 shadow-sm">
                  <p className="text-xs text-gray-500 uppercase font-semibold">Total</p>
                  <p className="text-2xl font-bold text-emerald-700">{reporte.length}</p>
                </div>
                <div className="bg-white rounded-lg p-3 shadow-sm">
                  <p className="text-xs text-gray-500 uppercase font-semibold">Abiertas</p>
                  <p className="text-2xl font-bold text-blue-700">{abiertas}</p>
                </div>
                <div className="bg-white rounded-lg p-3 shadow-sm">
                  <p className="text-xs text-gray-500 uppercase font-semibold">Cerradas</p>
                  <p className="text-2xl font-bold text-purple-700">{cerradas}</p>
                </div>
                <div className="bg-white rounded-lg p-3 shadow-sm">
                  <p className="text-xs text-gray-500 uppercase font-semibold">Total Cotizado</p>
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gradient-to-r from-emerald-700 to-emerald-600 text-white">
                    <tr>
                      ...
                      <th className="px-3 py-3 text-left font-semibold">No. Cotización</th>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {reporte.map((row, idx) => (
                      <tr key={idx} className={`hover:bg-emerald-50 transition-colors ${idx % 2 === 0 ? 'bg-white' : 'bg-gray-50/50'}`}>
                        ...
                        <td className="px-3 py-2 font-mono font-bold text-emerald-700">{row.no_Cotizacion}</td>
