...
export default function CotizacionFormPage() {
  ...
  const loadCotizacion = async (idCotizacion: number) => {
    ...
    try {
      ...
      setForm({
        tipo: cotizacion.tipo?.toString() || 'O',
      ...
      console.error('Error al cargar Conduce:', err);
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-amber-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="mt-2 text-emerald-800 text-sm">Cargando Conduce...</p>
        </div>
      </div>
    );
  }
          ...
          <div className="px-4 py-2 flex flex-wrap items-center justify-between gap-2">
            <div className="flex items-center gap-2">
              <h1 className="text-lg font-bold text-white">
                {esEdicion ? '✏️ Editar' : '📄 Nuevo'} Conduce
              </h1>
