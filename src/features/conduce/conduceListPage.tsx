...
const Tipo='O'  
  ...
  const loadCotizaciones = async () => {
    ...
    try {
      ...
      console.error('Error al cargar Conduces:', err);
  ...
  const handleDelete = async (id: number, numero: string) => {
    if (!confirm(`¿Eliminar el Conduce No. ${numero}?`)) return;
    try {
  ...
  const handleClose = async (id: number) => {
    if (!confirm('¿Cerrar este Conduce? No podrá modificarse después.')) return;
    try {
      await cotizacionApi.close(id);
      setCotizaciones(prev => prev.map(c => 
        c.idcotizacion === id ? { ...c, proceso: Tipo } : c
      ));
    } catch (err: any) {
      ...
      <div className="max-w-7xl mx-auto space-y-2">
        ...
        <div className="bg-gradient-to-r from-emerald-800 to-emerald-700 rounded-xl shadow-md overflow-hidden">
          <div className="px-4 py-2 flex flex-wrap items-center justify-between gap-2">
            <div>
              ...
              <p className="text-emerald-100 text-[11px]">Administra y da seguimiento a los Conduces de Ventas</p>
            </div>
            <button
              onClick={() => navigate('/Conduces/nuevo')}
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-2">
          ...
          <div className="bg-gradient-to-r from-yellow-500 to-orange-500 text-white rounded-lg p-2 shadow">
            <p className="text-[10px] opacity-90">Abiertos</p>
          ...
          <div className="bg-gradient-to-r from-gray-500 to-gray-600 text-white rounded-lg p-2 shadow">
            <p className="text-[10px] opacity-90">Cerrados</p>
          ...
          <div className="bg-gradient-to-r from-blue-500 to-blue-700 text-white rounded-lg p-2 shadow">
            <p className="text-[10px] opacity-90">Facturados</p>
          ...
          <div className="p-3">
            <div className="grid grid-cols-1 md:grid-cols-5 gap-2 items-end">
              <div>
                ...
                <input
                  type="text"
                  placeholder="Cliente o N° Conduce"
                  value={filters.texto}
        ...
        <div className="bg-white rounded-xl shadow-md border border-emerald-100 overflow-hidden">
          {loading ? (
            <div className="p-8 text-center text-gray-500 text-sm">Cargando Conduces...</div>
                    ...
                    <tr key={c.idcotizacion} className="border-b hover:bg-orange-50 transition-colors">
                      ...
                      <td className="px-3 py-2">
                        <div className="flex justify-center gap-1">
                          <button
                            onClick={() => navigate(`/Conduces/editar/${c.idcotizacion}`)}
                          ...
                          <button
                            onClick={() => navigate(`/Conduces/${c.no_Cotizacion}/imprimir/${Tipo}`)}
